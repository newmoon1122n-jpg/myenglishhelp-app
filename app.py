...
def translate_text(text, target_lang='zh-TW'):
    try:
        url = f"[https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=](https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=){target_lang}&dt=t&q={urllib.parse.quote(text)}"
...
def generate_cloze_sentences_free(vocabs):
    ...
    try:
        url = f"[https://text.pollinations.ai/](https://text.pollinations.ai/){urllib.parse.quote(prompt)}"
        with urllib.request.urlopen(url, timeout=12) as response:
            # 關鍵修正：確保移除可能存在的 markdown 標記，直接解析純 JSON
            return json.loads(re.sub(r'^```json\s*|\s*```$', '', response.read().decode('utf-8'), flags=re.MULTILINE).strip())
    except: return [{"target_word": v["word"], "new_sentence": f"We need to _______ more.", "meaning": v["meaning"], "distractors": random.sample(base_distractors, 3)} for v in vocabs]
...
if "quiz_vocabs" in query_params:
    ...
    # 移除 session_state 快取限制，確保每次進入都會產生新題目
